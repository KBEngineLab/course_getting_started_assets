import random

import KBEngine

from KBEDebug import DEBUG_MSG


class Monster(KBEngine.Entity):

	def __init__(self):
		KBEngine.Entity.__init__(self)
		DEBUG_MSG("Monster::__init__: created entityID = %i, spaceID = %s ,name = %s ,dict = %s." % ( self.id, self.spaceID,self.name,self.__dict__))

	def onEnterWorld(self):
		"""
		KBEngine method.
		如果实体非客户端控制实体，则表明实体进入了服务端上客户端控制的实体的View范围，此时客户端可以看见这个实体了。
		如果实体是客户端控制的实体则表明该实体已经在服务端创建了cell并进入了space。
		"""
		DEBUG_MSG("%s::onEnterWorld: %i" % (self.__class__.__name__, self.id))

	def onLeaveWorld(self):
		"""
		KBEngine method.
		如果实体非客户端控制实体，则表明实体离开了服务端上客户端控制的实体的View范围，此时客户端看不见这个实体了。
		如果实体是客户端控制的实体则表明该实体已经在服务端销毁了cell并离开了space。
		"""
		DEBUG_MSG("%s::onLeaveWorld: %i" % (self.__class__.__name__, self.id))


	def set_HP(self, oldValue):
		"""
		Property method.
		"""
		DEBUG_MSG("%s::set_HP: %i changed:%s->%s" % (self.__class__.__name__, self.id, oldValue, self.HP))


	def set_MP(self, oldValue):
		"""
		Property method.
		"""
		DEBUG_MSG("%s::set_MP: %i changed:%s->%s" % (self.__class__.__name__, self.id, oldValue, self.MP))
